...
- **Manage Tracking By**(NEW): If your orders only have 1 tracking number each at a time, you can choose to manage tracking by order only. This way, tracking number and carrier are saved to order post metas so you use these post meta keys to add integration with other plugins such as an email designer plugin, an SMS plugin...
...
- **Track Per Item Quantity**(NEW): There will a tracking number field for each purchased quantity of an order item. Helpful when the same item in an order can have different tracking numbers at a time.
...
- **Manage User Roles**(NEW): It's possible to grant Shop manager access to a specific settings of the plugin
